...
export interface UseProfileReturn {
  ...
  // Actions
  updateProfile: (data: { name?: string; phone?: string; location?: string }) => Promise<boolean>;
...
const activityCache = new Map<string, { data: ActivityEntry[]; timestamp: number }>();
const CACHE_DURATION = 5000; // 5 seconds (reduced for token updates)
const TOKEN_CACHE_DURATION = 2000; // 2 seconds for token-specific data
...
async function updateProfileAPI(data: { name?: string; phone?: string; location?: string }): Promise<ProfileData> {
  const response = await fetch("/api/profile", {
    method: "PATCH",
...
async function fetchActivity(userId: string, force = false): Promise<ActivityEntry[]> {
  // Check cache first
...
export function useProfile(): UseProfileReturn {
  ...
  const refresh = useCallback(async (force = false) => {
    ...
    const cacheDuration = force ? 0 : CACHE_DURATION;
  ...
  // Update profile
  const updateProfile = useCallback(async (data: { name?: string; phone?: string; location?: string }): Promise<boolean> => {
  ...
  // Get activity
  const getActivity = useCallback(async (force = false): Promise<ActivityEntry[]> => {
